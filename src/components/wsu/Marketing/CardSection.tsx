...
function CardComponent({ card }: { card: CardData }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0 opacity-0 group-hover:opacity-100 transition-opacity duration-1500 ease-in-out bg-gradient-to-b from-purple-500/10 via-transparent to-teal-500/10"></div>
      
      {/* Additional glow effects for featured card */}
      {isFeatured && (
        <>
          {/* Outer glow */}
          <div className="absolute -inset-0.5 bg-gradient-to-r from-purple-500/15 via-teal-500/15 to-purple-500/15 rounded-[24px] blur-sm z-0 opacity-60 transition-opacity duration-700 ease-in-out group-hover:opacity-80"></div>
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .animate-glow-float {
          animation: glow-float 10s cubic-bezier(0.25, 0.1, 0.25, 1) infinite;
        }
        ...
        .animate-glow-float-delayed {
          animation: glow-float-delayed 10s cubic-bezier(0.25, 0.1, 0.25, 1) infinite 2.5s;
        }
      `}</style>
      
      <div className="container px-4 sm:px-6 py-8 sm:py-12 md:py-20 lg:py-32 mx-auto">
        <div className="flex flex-col items-center text-center mb-16 sm:mb-20">
          <h2 className="text-5xl sm:text-6xl md:text-7xl font-bold bg-gradient-to-r from-purple-400 to-teal-400 bg-clip-text text-transparent mb-4">
            Our Platform
          </h2>
          <p className="text-lg text-gray-300 max-w-3xl">
            Discover how WeSeedU connects investors with sustainable startups making real impact
          </p>
        </div>
